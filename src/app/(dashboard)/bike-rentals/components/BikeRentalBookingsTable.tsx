...
interface BikeRentalBookingsTableProps {
  bookings: any[];
  isLoading: boolean;
  page: number;
  limit: number;
...
export function BikeRentalBookingsTable({
  bookings,
  isLoading,
  page,
  limit,
        ...
        if (!hasOptions) {
          return (
            <div className="flex justify-center">
              <Link href={`/bike-rentals/${row.id}/details?source=bookings`} className="rounded bg-white/5 border border-white/10 px-3 py-1.5 text-xs font-medium text-white hover:bg-white/10 transition-colors">
                View Details
              </Link>
            </div>
          );
