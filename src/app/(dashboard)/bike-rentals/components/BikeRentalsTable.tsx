...
import { BikeRentalBike } from '@/services/bike-rentals/bike-rentals.service';
import { ServerSideTable, type ColumnDef } from '@/components/ui/server-side-table';

interface BikeRentalsTableProps {
  bikes: BikeRentalBike[];
  isLoading: boolean;
  page: number;
  limit: number;
  total: number;
...
export function BikeRentalsTable({
  bikes,
  isLoading,
  page,
  limit,
  ...
  onLimitChange,
  onToggleStatus,
}: BikeRentalsTableProps) {
  const columns: ColumnDef<BikeRentalBike>[] = [
    {
      key: 'bike',
      title: 'Bike',
      dataIndex: 'name',
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center justify-center gap-3">
          <Link href={`/bike-rentals/${row.id}/edit`} className="text-gray-400 hover:text-white transition-colors" title="Edit Bike">
  ...
  return (
    <div className="rounded-lg border border-[#27272A] bg-[#111111] overflow-hidden">
      <ServerSideTable
        ...
        emptyText="No bikes found. Click 'Add Bike Rental' to create one."
      />
    </div>
  );
}
