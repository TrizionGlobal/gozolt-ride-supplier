...
import { BikeRentalBookingsTable } from './BikeRentalBookingsTable';
...
export function BookingsTab() {
  ...
  const fetchBookings = useCallback(async (force = false) => {
    ...
    try {
      const res = await bikeRentalsService.getBookings({ 
        page: managementPage, 
  ...
  const fetchExtensions = useCallback(async () => {
    ...
    try {
      const res = await bikeRentalsService.getExtensionRequests({ 
        page: managementPage, 
        limit: managementLimit, 
        search: managementSearch || undefined,
  ...
  const handleApproveExtension = async (id: string) => {
    const result = await Swal.fire({
      ...
      preConfirm: async () => {
        try {
          await bikeRentalsService.approveExtensionRequest(id);
          return true;
        } catch (e: any) {
  ...
  const handleRejectExtension = async (id: string) => {
    const result = await Swal.fire({
      ...
      preConfirm: async () => {
        try {
          await bikeRentalsService.rejectExtensionRequest(id);
          return true;
        } catch (e: any) {
  ...
  const handleUpdateStatus = async (id: string, status: string) => {
    try {
      await bikeRentalsService.updateBookingStatus(id, status);
      fetchBookings(true);
    } catch (e) {
      console.error(e);
