...
export function ExtensionRequestsTable({
  ...
  const handleViewDetails = async (row: any) => {
    ...
    if (row.bookingId) {
      ...
      try {
        const fullBooking = await carRentalsService.getBookingDetails(row.bookingId);
            ...
            <div className="p-6 overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-[#111111] border border-[#27272A] p-4 rounded-lg space-y-3">
                  <h5 className="text-xs uppercase text-[#FACC15] font-bold tracking-wider">Vehicle Details</h5>
                ...
                <div className="md:col-span-2 space-y-4">
                  ...
                  {loadingDetails ? (
                    ...
                    <div className="bg-[#111111] p-5 rounded-xl border border-[#27272A] text-sm space-y-3">
                      <div className="flex justify-between"><span className="text-gray-400">Vehicle Rate:</span> <span className="font-medium">€{Number(selectedRequest.booking?.vehicleTotal || 0).toFixed(2)}</span></div>
                      ...
                      {/* Dynamic Add-ons */}
                      {(selectedRequest.booking?.addons || selectedRequest.booking?.addOns)?.map((addon: any, idx: number) => (
                        <div key={idx} className="flex justify-between">
                          <span className="text-gray-400">{addon.name}:</span>
                          <span className="font-medium">€{Number(addon.price || addon.pricePerDay || 0).toFixed(2)}</span>
                        </div>
                      ))}

                      {/* Fallback for missing addon details but valid total */}
                      {!(selectedRequest.booking?.addons || selectedRequest.booking?.addOns) && Number(selectedRequest.booking?.addOnsTotal) > 0 && (
                        <div className="flex justify-between"><span className="text-gray-400">Add-ons Total:</span> <span className="font-medium">€{Number(selectedRequest.booking?.addOnsTotal || 0).toFixed(2)}</span></div>
                      )}
